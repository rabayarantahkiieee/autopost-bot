...
# ---------------- SUMBER BERITA ----------------
def extract_pl_image(c: dict) -> str:
    """Ambil URL gambar utama artikel PL, kalau ada."""
    try:
        lead = c.get("leadMedia") or {}
        # struktur umum API pulselive: leadMedia.variants[..].url
        for v in lead.get("variants", []):
            if v.get("url"):
                return v["url"]
        if lead.get("imageUrl"):
            return lead["imageUrl"]
        thumb = c.get("thumbnail") or {}
        for v in thumb.get("variants", []):
            if v.get("url"):
                return v["url"]
    except Exception:
        pass
